...
function generateConfigFile(){
    ...
    if (count >= 1)
    {
        ...
        if(document.getElementById("onDemandRadio").checked){
            scheduleId="0";
        }else if(document.getElementById("autoScheduleRadio").checked){  
            scheduleId=document.searchform.schedules.value;
            var profilerType= document.searchform.profilerType.value;        
            params=params+"&profilerType="+escape(profilerType);           
        }
        if(errString!=""){
            alert(errString);  
        }else{
            // alert(scheduleId);
            var url = "ajax";
            var ui = "generateConfigFile";
            params=params+"&scheduleId="+scheduleId+"&loadprofiledays="+escape(loadprofiledays)+"&loadprofiletype="+loadprofiletype+"&energydata="+energydata+"&uiaction="+ ui;    
            //alert(params);
            callAjax("POST",url,params,false,out.response);  
        }
    }
